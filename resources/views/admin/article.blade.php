...
<div class="table-responsive">
    <div class="table-wrapper">
        <div class="table-title">
            <div class="row">
                <div class="col-sm-6">
                    <h2>Page <b>apropos</b></h2>
                </div>
                <div class="col-sm-6">
                    <a href="{{route('admin.article.create')}}" class="btn btn-success"><i class="material-icons">&#xE147;</i> <span>Ajouter une ligne</span></a>
                </div>
            </div>
        </div>
        <table class="table table-striped table-hover">
            ...
            <tbody class="">					
                @foreach ($article as $item)
                    <tr class="card-sel">
                        <td><img src="data:image/jpeg;base64,{{$item->imgArt}}" width=25 height=25/></td>
                        <td>{{$item->title}}</td>
                        <td>{{$item->body}}</td>
                        <td class="d-flex">
                            <a href="{{route('admin.article.edit',['article'=>$item->id_art])}}" class="edit"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>
                            <form action="{{route('admin.article.destroy',['article'=>$item->id_art])}}" method="post">
                                @method('delete')
                                @csrf
